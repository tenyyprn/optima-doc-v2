...
const styles = {
  container: "p-2 sm:p-4 w-full h-screen overflow-hidden",
  row: "flex flex-col lg:flex-row w-full h-[calc(100vh-1rem)] sm:h-[calc(100vh-2rem)] gap-2 sm:gap-4",
  panel: "w-full flex flex-col",
  leftPanel: "lg:w-1/2 p-2 sm:p-4 overflow-hidden flex flex-col h-1/2 lg:h-full", // モバイルで高さ50%
  rightPanel: "lg:w-1/2 p-2 sm:p-4 flex flex-col h-1/2 lg:h-full", // モバイルで高さ50%
  header: "flex flex-col sm:flex-row sm:justify-between gap-2 mb-2 sm:mb-4 flex-shrink-0",
...
function OcrResult() {
  ...
  const rerunOcr = async () => {
    ...
    try {
      ...
      showToast('OCR処理を開始しました。完了までお待ちください...', 'info');
      ...
      console.error('OCR再実行エラー:', error);
      showToast('OCR再実行に失敗しました', 'error');
      setIsRerunning(false);
    }
  };
            ...
            {isMultipage && (
              <div className="flex items-center gap-1 sm:gap-2">
                <button
                  ...
                  className="px-2 sm:px-3 py-1 text-sm bg-gray-500 text-white rounded disabled:bg-gray-300 disabled:cursor-not-allowed hover:bg-gray-600"
                >
                  ← 前
                </button>
            ...
            <div className="flex flex-wrap gap-1 sm:gap-2">
              ...
              {!loading && appName && (
                <button
                  ...
                  className="bg-indigo-500 hover:bg-indigo-600 text-white px-2 py-1 sm:px-4 sm:py-2 rounded text-xs sm:text-base whitespace-nowrap"
                >
                  <span className="hidden sm:inline">カスタムプロンプト</span>
                  <span className="sm:hidden">プロンプト</span>
                </button>
              )}
              
              {/* OCR再実行ボタン（両方の画面で表示） */}
              {!loading && isOcrEnabled() && (
                <button
                  onClick={rerunOcr}
                  disabled={isRerunning}
                  className="bg-orange-500 hover:bg-orange-600 text-white px-2 py-1 sm:px-4 sm:py-2 rounded disabled:bg-gray-400 disabled:cursor-not-allowed text-xs sm:text-base whitespace-nowrap"
                >
                  {isRerunning ? (
                    <>
                      <span className="hidden sm:inline">OCR実行中...</span>
                  ...
                  activeView === "ocr" ? (
                    <>
                      <button
                        ...
                        className="bg-gray-500 text-white px-2 py-1 sm:px-4 sm:py-2 rounded text-xs sm:text-base whitespace-nowrap"
                      >
                        <span className="hidden sm:inline">抽出画面へ戻る</span>
                        <span className="sm:hidden">抽出画面</span>
                      </button>
                    </>
                  ) : (
                    <button
                      onClick={() => changeView("ocr")}
                      className="bg-blue-500 text-white px-2 py-1 sm:px-4 sm:py-2 rounded text-xs sm:text-base whitespace-nowrap"
                    >
                      <span className="hidden sm:inline">OCR結果を確認/編集する</span>
                      <span className="sm:hidden">OCR確認</span>
                    </button>
                  )
                )}
