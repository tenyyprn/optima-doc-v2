...
export interface ApiProps {
  ...
  schemasTable: Table;
  toolsTable?: Table;
  datacheckProjectsTable: Table;
  portCodesTable: Table;
  feedbackTable: Table;
  userPoolId: string;
  ...
  constructor(scope: Construct, id: string, props: ApiProps) {
    ...
    const lambdaFunction = new DockerImageFunction(this, "ApiFunction", {
      ...
      environment: {
        ...
        DATACHECK_PROJECTS_TABLE_NAME: props.datacheckProjectsTable.tableName,
        FEEDBACK_TABLE_NAME: props.feedbackTable.tableName,
    ...
    // データチェック用スキーマをS3にデプロイ
    const schemaDeployment = new BucketDeployment(
      this,
      "DeployDataCheckSchemas",
      {
    ...
    // データチェック用スキーマ登録Lambda関数
    const datacheckSchemaLoaderFunction = new PythonFunction(
      this,
      "DataCheckSchemaLoaderFunction",
      {
        entry: "lambda/datacheck-schema-loader",
        ...
        memorySize: 256,
        environment: {
          MASTER_DATA_BUCKET_NAME: documentBucket.bucketName,
          SCHEMAS_TABLE_NAME: props.schemasTable.tableName,
        },
      }
    );
    ...
    // 権限付与
    documentBucket.grantRead(datacheckSchemaLoaderFunction);
    props.schemasTable.grantReadWriteData(datacheckSchemaLoaderFunction);

    // カスタムリソースでスキーマを登録
    const datacheckSchemaLoader = new CustomResource(
      this,
      "DataCheckSchemaLoader",
      {
        serviceToken: datacheckSchemaLoaderFunction.functionArn,
      }
    );

    // デプロイ順序
    datacheckSchemaLoader.node.addDependency(schemaDeployment);

    // 港コードマスタデータのデプロイ
    const portCodeDataDeployment = new BucketDeployment(
      this,
      "PortCodeDataDeployment",
      {
        sources: [Source.asset("assets/data")],
        destinationBucket: documentBucket,
    ...
    // 港コードローダーLambda関数
    const portCodeLoaderFunction = new PythonFunction(
      this,
      "PortCodeLoaderFunction",
      {
        entry: "lambda/port-code-loader",
        runtime: Runtime.PYTHON_3_12,
        index: "index.py",
        handler: "handler",
        ...
        environment: {
          MASTER_DATA_BUCKET_NAME: documentBucket.bucketName,
          PORT_CODES_TABLE_NAME: props.portCodesTable.tableName,
        },
      }
    );

    // 権限付与
    documentBucket.grantRead(portCodeLoaderFunction);
    props.portCodesTable.grantReadWriteData(portCodeLoaderFunction);

    // カスタムリソースで港コードを登録
    const portCodeLoader = new CustomResource(this, "PortCodeLoader", {
      serviceToken: portCodeLoaderFunction.functionArn,
    });

    // デプロイ順序
    portCodeLoader.node.addDependency(portCodeDataDeployment);
  }
}
